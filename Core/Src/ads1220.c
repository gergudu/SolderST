 ...
 * раза в ~55 мс на канал (гейт по HAL_GetTick() внутри read_raw()),
 * неблокирующе.
 */
...
/* =========================================================================
 * Конфигурация регистров — подтверждена по реальной схеме (ток
 * IDAC1 течёт AIN3/REFN1 → RTD → Rref → GND; RTD измеряется
 * AIN1(+)/AIN0(-); Rref измеряется REFP0(+)/REFN0(-)), проверено
 * побитово по даташиту TI. Значения ниже НЕ менялись — только
 * комментарии, которые раньше были перепутаны местами (см. историю
 * коммитов).
 *
 * REG0 = 0x68: MUX=0110 -> AIN1(+)/AIN0(-) — верно, совпадает со
 *              схемой. GAIN=100 -> x16, PGA вкл. — совпадает с
 *              ADS1220_PGA_GAIN=16.0f, реально участвующим в расчёте.
 *              (неиспользуемый декоративный макрос ADS1220_REG0_GAIN_1
 *              в хэдере неверно назван и не участвует в сборке
 *              REG0_VAL — оставлен как есть, не мешает)
 * REG1 = 0x04: 20 SPS, нормальный режим, непрерывное преобразование
 * REG2 = 0x55: VREF=01 -> внешний REFP0/REFN0 — верно, совпадает со
 *              схемой. IDAC=101 -> 1000 мкА (не 500мкА, как было в
 *              старом комментарии и в ADS1220_IDAC_UA хэдера — тот
 *              макрос декоративный, в расчёте не участвует). На расчёт
 *              температуры не влияет — измерение рациометрическое,
 *              ток через Rref и RTD одинаковый и сокращается в
 *              отношении, но фактический самонагрев RTD вдвое выше
 *              документированного.
 * REG3 = 0x80: IDAC1 -> AIN3 — верно, совпадает со схемой
 * ========================================================================= */
